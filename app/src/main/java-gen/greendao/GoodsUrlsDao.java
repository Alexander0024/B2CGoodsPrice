...
public class GoodsUrlsDao extends AbstractDao<GoodsUrls, Long> {
    ...
    */
    ...
    private Query<GoodsUrls> goods_UrlsQuery;
    ...
    /** Internal query to resolve the "urls" to-many relationship of Goods. */
    public List<GoodsUrls> _queryGoods_Urls(Long goodsId) {
        synchronized (this) {
            if (goods_UrlsQuery == null) {
                QueryBuilder<GoodsUrls> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.GoodsId.eq(null));
                goods_UrlsQuery = queryBuilder.build();
            }
        }
        Query<GoodsUrls> query = goods_UrlsQuery.forCurrentThread();
